...
import com.eltescode.notes.features.note.domain.use_case.NoteUseCases
...
class NotesViewModel @Inject constructor(private val noteUseCases: NoteUseCases) : ViewModel() {
    ...
    private var recentlyDeletedNote: NoteDomain? = null
        ...
        when (event) {
            is NotesEvent.DeleteNote -> {
                viewModelScope.launch {
                    noteUseCases.deleteNoteUseCase(event.noteDomain)
                    recentlyDeletedNote = event.noteDomain
                }
            }
            is NotesEvent.Order -> {
